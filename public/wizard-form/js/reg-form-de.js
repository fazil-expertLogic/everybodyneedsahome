...
*/	
	$("#signUpForm").validator().on("submit", function (event) {
		if (event.isDefaultPrevented()) {
			...
			if (inputHumanAns == correctMathSolution){
				...
				submitMSG(false, "Bitte lösen Sie menschliches Captcha!!!");
				sweetAlert("Oops...", "Bitte lösen Sie menschliches Captcha!!!", "error");
				return false;
			}
		}
	});
	...
	function formSuccess(){
		$("#signUpForm")[0].reset();
		submitMSG(true, "Registrierungsprozess erfolgreich!")
	}
...
*/				
	...
	function nextStep2() {
		...
		if( uname )
			...
			$( ".validuname .help-block.with-errors" ).html( '<ul class="list-unstyled"><li>Bitte eingeben Benutzername</li></ul>' );
		
		if( validemail )
			$( ".validemail .help-block.with-errors" ).html( '' );
		else	
			$( ".validemail .help-block.with-errors" ).html( '<ul class="list-unstyled"><li>Bitte eingeben gültig email</li></ul>' );
		
		if(  pass.length < 6 )
			$( ".validpass .help-block.with-errors" ).html( '<ul class="list-unstyled"><li>Passwort mindestens 6 Zeichen</li></ul>' );
		else if(  pass != cpass ) {
			$( ".validpass .help-block.with-errors" ).html( '<ul class="list-unstyled"><li>Kennwort und Kennwort bestätigen nicht</li></ul>' );
			sweetAlert("Oops...", "Kennwort und Kennwort bestätigen nicht!!!", "error");return false;
		}
		else
		...
		if( uname && validemail && pass.length > 5 && pass == cpass ) {
			...
			$( "#section-1 .help-block.with-errors.mandatory-error" ).html( '<ul class="list-unstyled"><li>Bitte füllen Sie das Formular ordnungsgemäß aus</li></ul>' );
			sweetAlert("Oops...", "Bitte füllen Sie das Formular ordnungsgemäß aus!", "error");
		}
	}
	...
	function nextStep3() {
		...
		if( fname )
			...
			$( ".validfname .help-block.with-errors" ).html( '<ul class="list-unstyled"><li>Bitte eingeben Vorname</li></ul>' );
		
		if( lname )
			$( ".validlname .help-block.with-errors" ).html( '' );
		else
			$( ".validlname .help-block.with-errors" ).html( '<ul class="list-unstyled"><li>Bitte eingeben Nachname</li></ul>' );
		
		if( gender )
			$( ".validgender .help-block.with-errors" ).html( '' );
		else
			$( ".validgender .help-block.with-errors" ).html( '<ul class="list-unstyled"><li>Bitte Wählen Geschlecht</li></ul>' );
		
		if( birthdate )
			$( ".validbirthdate .help-block.with-errors" ).html( '' );
		else
			$( ".validbirthdate .help-block.with-errors" ).html( '<ul class="list-unstyled"><li>Bitte eingeben Geburtsdatum</li></ul>' );
		
		if( address )
			$( ".validaddress .help-block.with-errors" ).html( '' );
		else
			$( ".validaddress .help-block.with-errors" ).html( '<ul class="list-unstyled"><li>Bitte eingeben Adresse</li></ul>' );
		...
		if (filter.test(phone)) {
			...
			$( ".validphone .help-block.with-errors" ).html( '<ul class="list-unstyled"><li>Bitte eingeben gültig Telefon</li></ul>' );
			var validphone = 0;
		}
		...
		if( preferedcontact )
			...
			$( ".validpreferedcontact .help-block.with-errors" ).html( '<ul class="list-unstyled"><li>Bitte wählen Sie bevorzugte Kontaktmethode</li></ul>' );
		...
		if( fname.length > 0 && fname && lname.length > 0 && lname && gender && birthdate.length > 4 && birthdate && phone.length > 4 && validphone > 0 && address.length > 0 && address && preferedcontact ) {
			...
			$( "#section-2 .help-block.with-errors.mandatory-error" ).html( '<ul class="list-unstyled"><li>Bitte füllen Sie das Formular ordnungsgemäß aus</li></ul>' );
			sweetAlert("Oops...", "Bitte füllen Sie das Formular ordnungsgemäß aus!", "error");
		}		
	}
	...
	function nextStep4() {
		...
		//write inputed data
		$( "#unameData" ).html( '<strong>Benutzername:</strong> '+ uname );
		$( "#emailData" ).html( '<strong>email:</strong> '+ email );
		$( "#passData" ).html( '<strong>Passwort:</strong> *****' );
		$( "#firstNameData" ).html( '<strong>Vorname:</strong> '+ fname );
		$( "#lastNameData" ).html( '<strong>Nachname:</strong> '+ lname );
		$( "#genderData" ).html( '<strong>Geschlecht:</strong> '+ gender );
		$( "#birthdateData" ).html( '<strong>Geburtsdatum:</strong> '+ birthdate );
		$( "#addressData" ).html( '<strong>Adresse:</strong> '+ address );
		$( "#phoneData" ).html( '<strong>Telefon:</strong> '+ phone );
		$( "#preferedcontactData" ).html( '<strong>Bevorzugte Kontaktmethode:</strong> '+ preferedcontact );
		$( "#paymenttypeData" ).html( '<strong>Zahlungsart:</strong> '+ paymenttype );
		$( "#hnameData" ).html( '<strong>Name des Karteninhabers:</strong> '+ hname );
		$( "#cardnumberData" ).html( '<strong>Kartennummer:</strong> '+ cardnumber );
		$( "#cvcData" ).html( '<strong>CVC:</strong> ***' );
		$( "#expirydateData" ).html( '<strong>Verfallsdatum:</strong> '+ expirydate );
		
		if( paymenttype )
			$( ".validpaymenttype .help-block.with-errors" ).html( '' );
		else
			$( ".validpaymenttype .help-block.with-errors" ).html( '<ul class="list-unstyled"><li>Bitte wählen Sie Zahlungsart</li></ul>' );
		
		if( hname )
			$( ".validhname .help-block.with-errors" ).html( '' );
		else
			$( ".validhname .help-block.with-errors" ).html( '<ul class="list-unstyled"><li>Bitte geben Sie den Karteninhabernamen ein</li></ul>' );
		...
		if( cardnumber && re16digit.test(cardnumber) )
			...
			$( ".validcardnumber .help-block.with-errors" ).html( '<ul class="list-unstyled"><li>Bitte geben Sie 16-stellige Kartennummer ein</li></ul>' );
			return false;
		}
		...
		if( cvc )
			...
			$( ".validcvc .help-block.with-errors" ).html( '<ul class="list-unstyled"><li>Bitte eingeben CVC</li></ul>' );
		
		if( expirydate )
			$( ".validexpirydate .help-block.with-errors" ).html( '' );
		else
			$( ".validexpirydate .help-block.with-errors" ).html( '<ul class="list-unstyled"><li>Bitte geben Sie das Gültigkeitsdatum ein</li></ul>' );
		
		if( $('#aggre').is(":checked") )
			$( ".validagree .help-block.with-errors" ).html( '' );
		else{
			$( ".validagree .help-block.with-errors" ).html( '<ul class="list-unstyled"><li>Bitte vereinbaren Sie die AGBs</li></ul>' );
			sweetAlert("Oops...", "Bitte vereinbaren Sie die AGBs!", "error");return false;
		}
		...
		if( $('#aggre').is(":checked") && paymenttype && hname && cardnumber && cvc && expirydate ) {
			...
			$( "#section-3 .help-block.with-errors.mandatory-error" ).html( '<ul class="list-unstyled"><li>Bitte füllen Sie das Formular ordnungsgemäß aus</li></ul>' );
			sweetAlert("Oops...", "Bitte füllen Sie das Formular ordnungsgemäß aus!", "error");
		}
