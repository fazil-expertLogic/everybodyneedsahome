        ...
        <div class="col-lg-12">
            <div class="card">
                ...
                <div class="mb-3 text-end">
                    @if ($allow_create)
                        <a href="{{ route('pageContents.create') }}" class="btn bg-primary btn-sm badge"
                ...
                <form action="{{ route('pageContents.index') }}" method="GET">
                    <div class="mb-3 text-end">
                        <div class="input-group w-100">
                            ...
                            <div class="col">
                                <input type="text" name="page_url" class="form-control" placeholder="Page Url"
                                    value="{{ request('page_url') }}">
                            </div>

                            <div class="col">
                                <input type="text" name="variable" class="form-control" placeholder="Variable"
                            ...
                            <tbody>
                                @foreach ($pageContents as $pageContent)
                                    <tr>
                                        ...
                                        <td>
                                            @if ($allow_show)
                                                <a href="{{ route('pageContents.show', $pageContent->id) }}"
                                                    class="btn btn-warning btn-sm badge" title="Show">
                                                    <i class="fa fa-eye" aria-hidden="true"></i>
                                                </a>
                                            @endif
                                            @if ($allow_edit)
                                                <a href="{{ route('pageContents.edit', $pageContent->id) }}"
                                ...
                                <nav aria-label="Page navigation example">
                                    <ul class="pagination justify-content-center mb-0">
                                        ...
                                        @for ($i = 1; $i <= $pageContents->lastPage(); $i++)
                                            <li
                                                class="page-item {{ $pageContents->currentPage() == $i ? 'active' : '' }}">
                                                <a class="page-link"
                                                    href="{{ $pageContents->url($i) }}">{{ $i }}</a>
                                            </li>
                                        @endfor
    ...
    <div class="modal fade" id="import" tabindex="-1" aria-labelledby="formModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ route('pageContents.import') }}" method="POST" enctype="multipart/form-data"
                    id="modalForm">
                    @csrf
                    <div class="modal-body">
                        <div class="mb-3">
