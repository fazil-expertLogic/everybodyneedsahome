    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="card">
                        ...
                        <div class="input-group w-100">
                            ...
                            <div class="col">
                                <input type="text" name="reviewer_email" class="form-control"
                                    placeholder="Reviewer Email" value="{{ request('reviewer_email') }}">
                            </div>
                            <div class="col">
                ...
                {{--  --------------------------------- export ------------------------------------- --}}
                <form action="{{ route('propertyReview.export') }}" method="get">
                    <div class="mb-3 text-end">
                        <div class="input-group w-100">
                            <div class="col">
                        ...
                        <table class="table table-bordered text-nowrap border-bottom" id="responsive-datatable">
                            ...
                            <tbody>
                                @foreach ($propertyReviewes as $propertyReview)
                                    <tr>
                                        <td>{{ $propertyReview->property ? $propertyReview->property->property_name : '' }}
                                        </td>
                                        ...
                                        <td>
                                            @if ($allow_show)
                                                <a href="{{ route('propertyReview.show', $propertyReview->id) }}"
                                                    class="btn btn-warning btn-sm badge" title="Show">
                                                    <i class="fa fa-eye" aria-hidden="true"></i>
                                                </a>
                                            @endif
                                            @if ($allow_edit)
                                                <a href="{{ route('propertyReview.edit', $propertyReview->id) }}"
                                            ...
                                            @if ($allow_delete)
                                                <button class="btn btn-danger btn-sm badge" title="Delete"
                                                    onclick="confirmDelete('{{ route('propertyReview.destroy', $propertyReview->id) }}');">
                                ...
                                <nav aria-label="Page navigation example">
                                    <ul class="pagination justify-content-center mb-0">
                                        ...
                                        @for ($i = 1; $i <= $propertyReviewes->lastPage(); $i++)
                                            <li
                                                class="page-item {{ $propertyReviewes->currentPage() == $i ? 'active' : '' }}">
                                                <a class="page-link"
                                                    href="{{ $propertyReviewes->url($i) }}">{{ $i }}</a>
                                            </li>
                                        @endfor
    ...
    <div class="modal fade" id="import" tabindex="-1" aria-labelledby="formModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ route('propertyReview.import') }}" method="POST" enctype="multipart/form-data"
                    id="modalForm">
                    @csrf
                    <div class="modal-body">
                        <div class="mb-3">
